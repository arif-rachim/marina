...
const createTableRowFromData = (displayRaw,data,attributes,resources) => {
    let row = [];
    row.push(`<td>
    <button class="mdl-button mdl-js-button mdl-button--icon">
        <a href="/v1/${resources}/${data._id}?intent=update">
...
const createTableBody = (displayRaw,attributes,datas,resourceName) => {
    return datas.map((data,index) => {
        return `
        <tr>
            ${createTableRowFromData(displayRaw,data,attributes,resourceName)}
        </tr>
        `
    }).join('');
}
    ...
    <div style="margin-top: 1em;text-align: center">
        <div style="display: inline-block;margin: auto;text-align: left;">
            <table class="mdl-data-table mdl-js-data-table mdl-shadow--2dp" >
                ...
                <tbody>
                    ${createTableBody(displayRaw,resourceDesign.attributes,datas,resource)}
                </tbody>
            </table>

...
const {render} = require('./number-render');
...
class Number {
    ...
    buildModel(){
        ...
        return {
            ...
            minValue : {
                value : this.input.hasAttribute('min') ? this.input.getAttribute('min') : 0,
                    name : 'Minimum Value',
                    type : 'number',
                    description : 'If selected, minimum value validation will be triggered before the form submitted'
            },
            maxValue : {
                value : this.input.hasAttribute('max') ?  this.input.getAttribute('max') : 1000000000000,
                    name : 'Maximum Value',
                    type : 'number',
                    description : 'If selected, maximum value validation will be triggered before the form submitted'
            },
            step : {
                value : this.input.hasAttribute('step') ?  this.input.getAttribute('step') : 0.01,
                name : 'Number Step',
                type : 'number',
                description : 'If selected, this is the value of step function'
            },
            pattern : {
    ...
    isValid(){
        ...
        return new Promise(resolve => {
            ...
            if(this.model.minValue.value > 0){
                if(value < this.model.minValue.value){
                    input.setCustomValidity(`Minimum value ${this.model.label.value} is ${this.model.minValue.value}`);
                    resolve({success : false,message:input.validationMessage});
                    return;
                }
            }
            if(this.model.maxValue.value > 0){
                if(value > this.model.maxValue.value){
                    input.setCustomValidity(`Maximum value ${this.model.label.value} is ${this.model.maxValue.value}`);
                    resolve({success : false,message:input.validationMessage});
                    return;
                }
            }
    ...
    refreshModel() {
        ...
        if(this.model.minValue.value){
            input.setAttribute('min',this.model.minValue.value);
        }else{
            input.removeAttribute('min');
        }

        if(this.model.maxValue.value){
            input.setAttribute('max',this.model.maxValue.value);
        }else{
            input.removeAttribute('max');
        }

        if(this.model.step.value){
            input.setAttribute('step',this.model.step.value);
        }else{

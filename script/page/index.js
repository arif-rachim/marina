...
const getArticleTags = async () => {
    let articles = await fetch(`/res/articles?$p.tags=1`);
    if(articles.docs){
...
const renderArticles = async (req) => {
    let articles = await fetch(`/res/articles?$i=5&$l=20&$s._createdOn=-1`);
...
const renderLatestPost = async (req) => {
    let articles = await fetch(`/res/articles?$i=0&$l=5&$s._createdOn=-1`);
    articles = articles.docs;
    let maxChars = 250;
    let renderArticle = (article) => {
...
const renderHighlightStories = async (req) => {
    let articles = await fetch(`/res/articles?$i=25&$l=5&$s._createdOn=-1`);
    articles = articles.docs;
    let maxChars = 150;
    let renderArticle = (article) => {
        let content = article.content ? article.content.replace(/<[^>]*>/g, "") : '';

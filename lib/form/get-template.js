const render = (attributes) => {
    return attributes.map(attribute => {
        if(attribute.type == 'boolean'){
            return `
            <vaadin-checkbox name="${attribute.name}" >${attribute.label}</vaadin-checkbox>
            `;
        }
        if(attribute.type == 'number'){
            return `
            <vaadin-text-field label="${attribute.label}" ${attribute.required ? 'required' : ''} name="${attribute.name}" prevent-invalid-input pattern="[0-9]+(\\.[0-9][0-9]?)?"></vaadin-text-field>
            `;
        }

        if(attribute.type == 'date'){
            return `
            <vaadin-date-picker label="${attribute.label}" ${attribute.required ? 'required' : ''} name="${attribute.name}"></vaadin-date-picker>
            `;
        }

        return `
            <vaadin-text-field label="${attribute.label}" ${attribute.required ? 'required' : ''} name="${attribute.name}" ></vaadin-text-field>
        `;
    }).join(' ');
}

module.exports = ({resource,attributes}) => {
    return `
<!doctype html>
<html lang="en">
<head>
